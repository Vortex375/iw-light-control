...
export class ArduinoControl extends Service {
  ...
  async start(config: ArduinoControlConfig) {
    if (typeof config.port === 'string') {
      ...
      log.debug({ports}, 'serial port listing');
  ...
  setupPort(path: string, baudRate: number) {
    ...
    this.port = new SerialPort(path, {
      ...
      if (err) {
        log.error({err, path}, 'error opening serial port');
    ...
    this.port.on('error', (err) => {
      log.error({err}, 'serial port error');
      this.stop();
    });
  ...
  doWrite() {
    ...
    async.series([
      ...
      (cb) => {
        ...
        log.debug({len: buf.length, free }, 'wrote', buf.length, 'bytes');
        if ( ! free) {
      ...
      if (err) {
        log.error({err}, 'Error writing to serial port');
